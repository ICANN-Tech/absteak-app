import { m as ensure_array_like, h as head, t as attr_class, e as escape_html, v as attr, w as clsx, b as bind_props, p as pop, c as push, x as stringify } from "../../../chunks/index2.js";
  ...
  if (won || data.answers.length >= 6) {
    ...
    for (let $$index_3 = 0, $$length = each_array_2.length; $$index_3 < $$length; $$index_3++) {
      ...
      for (let index = 0, $$length2 = each_array_3.length; index < $$length2; index++) {
        ...
        $$payload.out.push(`<button${attr("data-key", letter)}${attr_class(clsx(classnames[letter]), "svelte-1pg2j5l")}${attr("disabled", submittable, true)} formaction="?/update" name="key"${attr("value", letter)}${attr("aria-label", `${stringify(letter)} ${stringify(description[letter] || "")}`)}>${escape_html(letter)}</button>`);
      }
      $$payload.out.push(`<!--]--></div>`);
    }
    $$payload.out.push(`<!--]--></div>`);
