<script lang="ts">
  ...
  import Button from '$lib/components/atoms/button/Index.svelte';
  ...
  function selectMenu(menu: string): void {
    ...
    switch(menu) {
      case 'CS':
        menuResponse = 'Customer Service: How can I assist you today? Feel free to ask about our services, reservations, or any concerns.';
        break;
      case 'Menu':
        menuResponse = 'Our Menu: We offer a variety of delicious dishes including appetizers, main courses, desserts, and beverages. What would you like to know about our menu?';
        break;
      case 'Dine In':
        menuResponse = 'Dine In: Experience our cozy atmosphere! We offer table service with our full menu. Would you like to make a reservation?';
        break;
      case 'Dine Out':
        menuResponse = 'Dine Out: Take our delicious food with you! We offer takeaway and delivery services. What would you like to order?';
        break;
    }
...
<div class="fixed bottom-6 right-6 z-50">
  ...
  {#if open}
    <div 
      class="chat-dialog fixed bottom-4 right-4 z-50 w-92 max-w-[95vw] h-[35rem] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-primary-200" 
    ...
    >
      ...
      <div class="relative h-32 bg-gradient-to-r from-primary-600 to-primary-500 rounded-t-2xl overflow-hidden">
        <!-- Background Image Overlay -->
        <div class="absolute inset-0 bg-cover bg-center opacity-30" style="background-image: url('/assets/chat_bot_bg.png')"></div>
        
        <!-- Header Content -->
        <div class="relative z-10 flex items-start justify-end px-4 py-3 h-full">
          <a href="#" class="transform transition duration-300 hover:scale-110">
            <img src="https://ui-avatars.com/api/?name=Samule&background=663618&color=f0e9e9&rounded=true" class="w-12 h-12" alt="" srcset="">
          </a>
        </div>
      </div>
      ...
      <div class="px-4 py-3 bg-primary-50 border-b border-primary-100">
        <div class="grid grid-cols-4 gap-2">
          {#each ['CS', 'Menu', 'Dine In', 'Dine Out'] as menuItem}
            <button
              class="px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 transform hover:scale-105"
      ...
      <!-- Input -->
      <div class="flex items-center gap-3 px-4 py-4 bg-white border-t border-primary-100 shadow-inner rounded-b-2xl">
        <input 
          type="text" 
          class="flex-1 rounded-full px-4 py-3 border border-primary-300 focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent bg-primary-50 transition-all duration-200 placeholder-primary-400" 
