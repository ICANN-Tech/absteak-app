<script lang="ts">
  ...
  // Generate textarea classes using UI constants (reuse input classes)
  $: textareaClasses = getInputClasses(size, variant, {
    error: !!error,
    success: !!success,
    disabled,
    readonly
...
<div class="flex flex-col gap-2">
  ...
  <div class="relative">
    <Textarea
      ...
      {rows}
      {maxlength}
      class="{textareaClasses} {variant === 'elegant' ? 'elegant-variant' : ''} resize-{resize}"
      {...$$restProps}
    />
  </div>
  ...
  <div class="flex justify-between items-center">
    ...
    {#if maxlength}
      <div class="text-sm {isOverLimit ? 'text-red-400' : isNearLimit ? 'text-amber-400' : variant === 'elegant' ? 'text-gray-400' : 'text-gray-500'}">
