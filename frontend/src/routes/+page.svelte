<script lang="ts">
	...
	$: currentSectionIndex = sections.findIndex(
		(section) => section.id === $viewportState.section.currentSection
	);
	...
	$: {
		if (currentSectionIndex !== lastSectionIndex) {
			navigationDirection = currentSectionIndex > lastSectionIndex ? 'down' : 'up';
...
<main class="bg-primary-700/5 min-h-screen">
	{#if $sectionsInitialized}
		{#each sections as section, index}
			...
			{@const slideOffset = isActive ? '0' : (index > currentSectionIndex ? '100vh' : '-100vh')}
			<div
				id={section.id}
				class="section-container {isActive ? 'active' : ''}"
				style="
